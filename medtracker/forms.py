...
def input_choices(field, ul_class="", **kwargs):
    ...
    for key, value in choices.items():
        html.append(
            u'<div class="btn-group"><input class="form-control" type="text" name="%s" value="%s"/><a href="#" class="delete" onclick="deleteParent(this);"><span class="glyphicon glyphicon-remove-circle"></span></a></div>'
            % (field_id, value)
        )
    html.append(
        u'<div class="%s-container" %s></div>'
        % (field_id, html_params(id=field_id, class_=ul_class))
    )
    html.append(
        u'<button class="btn btn-primary add_form_field">Add Option &nbsp; <span class="add_form_field">+ </span></button></div>'
    )
    return HTMLString(u"".join(html))
...
class QuestionView(MethodView):
    ...
    def getField(self, question):
        ...
        if kind == "text":
            ...
            return (
                qtype,
                TextAreaField(qlabel, render_kw={"placeholder": "Your answer"}),
            )
        if kind == "yes-no":
            return (
                qtype,
                RadioField(qlabel, choices=[(1, "Yes"), (0, "No")], coerce=int),
            )
        if kind == "numeric":
            ...
            return (
                qtype,
                IntegerRangeField(
                    qlabel, [validators.NumberRange(min=1, max=9)], default=1
                ),
            )
        if kind == "select":
        ...
        if kind == "radio":
            ...
            return (
                qtype,
                RadioField(
                    qlabel, choices=[(a, b) for a, b in choices.items()], coerce=int
                ),
            )
...
class TriggerConditionForm(ModelForm):
    ...
    subject = QuerySelectField(
        "Subject",
        query_factory=possible_question,
...
class PatientForm(Form):
    ...
    year = SelectField(
        ...
        choices=[(i, i) for ix, i in enumerate(range(2020, 2029))],
        coerce=int,
    )
