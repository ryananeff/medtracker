    ...
    if question.triggers:
        for trigger in question.triggers:
            ...
            for ix, c in enumerate(trigger.conditions):
                trigger_state = False
                subject_response = QuestionResponse.query.filter_by(
                    survey_response_id=response.survey_response_id,
                    question_id=c.subject_id,
                ).first()
                c_value = c.condition_value.strip().lower()
                if subject_response != None:
                    subject_response = (
                        subject_response._response.strip().lower()
                    )  # get the response inputted for the trigger condition
                else:
                    continue  # there was no response for that trigger condition...
                sr_float = (
                    float(subject_response)
                    if isfloat(subject_response)
...
def phone_trigger(message, recipients, callback=None):
    ...
    for r in recipients.split(";"):
        call = client.calls.create(
            url="https://suretify.co/triggers/phone?m=" + urllib.quote(message),
            to=r,
            from_=twilio_number,
        )
    return None
...
def sms_trigger(message, r, callback=None):
    call = client.messages.create(body=message, to=r, from_=twilio_number)
    return call
    ...
    if p.phone != None:
        if len(p.phone[-10:]) == 10:
            ...
            hello += (
                ...
                + url_for("start_survey", survey_id=survey.id, _external=True)
            )
...
def save_basic_response(message, uniq_id, question_id, session_id=None):
    _response = QuestionResponse(message, uniq_id, session_id, question_id)
...
def email_trigger(message, recipients, callback=None):
    for r in recipients.split(";"):
        ...
        with app.app_context():
            html = render_template("trigger_email.html", body=message)
...
def autocomplete_choices():
    ...
    for survey in surveys:
        ...
        for ix, question in enumerate(survey._questions):
            possible.append(
                {"name": "question." + str(question.id), "content": question.body}
            )
...
def prod_patient(prod_type, patient_id):
    ...
    if len(current_tasks) == 0:
        ...
        if prod_type == "sms":
            ...
            call = client.calls.create(
                url="https://suretify.co/triggers/voice?say_hello=1",
                to=recipients,
                from_=twilio_number,
            )
        elif prod_type == "email":
            questions = [
                i.id for i in Survey.query.get_or_404(last_task.task).questions
            ]
            url = (
                "https://suretify.co/surveys/serve/%s?question=%s&u=%s"
                % last_task.task,
                questions[last_task.iterator],
                last_task.parent_id,
            )
...
@app.route(
    "/send_survey/<method_type>/<int:patient_id>/<int:survey_id>",
    methods=["GET", "POST"],
)
@flask_login.login_required
def send_survey(method_type, patient_id, survey_id):
    survey = Survey.query.filter_by(
        id=survey_id, user_id=current_user.id
    ).first_or_404()
    ...
    if method_type == "voice":
        ...
        try:
            url = (
                "https://suretify.co"
                + url_for("start_survey", survey_id=survey.id)
                + "?u=%s" % patient.id
            )
            message = "Please complete the following survey at this link: %s" % url
            task = Progress(user=patient.email, task=survey.id, parent_id=patient.id)
            db_session.add(task)
            db_session.commit()
