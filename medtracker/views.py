...
def randomword(length):
    ...
    return "".join(
        random.choice("23456789ABCDEFGHIJKLMNPQRSTUVWXYZ") for i in range(length)
    )
...
def detect_user_session():
    ...
    if (patient_ident_req is None) | (g.device is None):
        ...
        def remember_pt_id(response):
            response.set_cookie(
                "patient_ident", g.patient_ident, max_age=datetime.timedelta(weeks=52)
            )
...
def user_loader(
    user_id,
):  # used by Flask internally to load logged-in user from session
...
def login():  # not logged-in callback
    ...
    if form.validate_on_submit():
        ...
        if user == None:
            ...
            render_template(
                "form_login.html", form=form, action="Please log in", data_type=""
            )
        if user.active == False:
            msg = Markup(
                "Your account is currently deactivated until an administrator activates it."
            )
            flash(msg)
...
def signup():
    ...
    if form.validate_on_submit():
        ...
        flash(
            "Successfully registered. Your account is currently inactive. You must wait for another current admin to grant your admin access."
        )
        return redirect(url_for("login"))
    ...
    return render_template(
        "form_signup.html",
        form=form,
        action="Sign up for ISMMS Health Check",
        data_type="",
    )
...
def change_password():
    ...
    return render_template(
        "change_password.html", form=form, action="Change password", data_type=""
    )
...
def index():
    ...
    if g.patient == None:
        flash(
            "Your device appears to be unregistered. Please <a href='/patients/signup/1'>register</a> your device."
        )
...
def add_survey():
    ...
    if request.method == "POST" and formobj.validate():
        ...
        return redirect(url_for("serve_survey_index"))
    return render_template(
        "form.html", action="Add", data_type="a survey", form=formobj
    )
...
def edit_survey(_id):
    ...
    if request.method == "POST":
        ...
        return redirect(url_for("serve_survey_index"))
    return render_template(
        "form.html", action="Edit", data_type=survey.title, form=formout
    )
...
def start_survey(survey_id):
    ...
    patients = None
    try:
        survey.description_html = delta_html.render(
            json.loads(survey.description)["ops"]
        )
    except:
        survey.description_html = "<p>" + survey.description + "</p>"
    return render_template(
        "start_survey.html", survey=survey, u=u, s=session_id, patients=patients
    )
...
def serve_survey(survey_id):
    ...
    previous_responses = SurveyResponse.query.filter(
        SurveyResponse.uniq_id == g.patient.id,
        SurveyResponse.end_time.isnot(None),
        SurveyResponse.start_time > today,
    ).first()
    if previous_responses != None:
        return render_template(
            ...
            survey=survey,
            patient=g.patient,
            message="You can only take the survey once per day.",
        )
    ...
    if survey_response_id == None:
        ...
        survey_response = SurveyResponse(
            survey_id=survey.id, uniq_id=uniq_id, session_id=sess, user_id=curuser
        )
    ...
    if (request.method == "POST") & len(request.form.getlist("response")) != 0:
        print("saving...")
        next_question, next_survey, exit, complete, message = save_response(
            request.form,
            question_id,
            session_id=sess,
        ...
        if next_survey != None:
            return redirect(
                url_for("start_survey", survey_id=trigger_survey, u=uniq_id, s=sess)
            )
        if exit:
            ...
            db_session.commit()
            return redirect(
                url_for("exit_survey", session_id=survey_response.session_id)
            )
        if complete:  ##complete survey!
            ...
            db_session.commit()
            return redirect(
                url_for("complete_survey", session_id=survey_response.session_id)
            )
        return redirect(
            url_for(
                "serve_survey",
        ...
        try:
            question.description_html = delta_html.render(
                json.loads(question.description)["ops"]
            )
        except:
...
def reset_device():
    ...
    if g.patient_ident != None:
        ...
        g.patient = None
        flash(
            "Device ID reset. You will need to register with ISMMS Health Check again to complete surveys."
        )
    return response
...
def exit_survey(session_id):
    ...
    if g.patient:
        ...
        return abort(
            401,
            "Your device appears to be unregistered. Only registered devices can view exit records.",
        )
...
def complete_survey(session_id):
    ...
    if current_user.is_authenticated:
        if record.completed:
            patient = record.patient
            end_time = record.end_time.replace(tzinfo=pytz.utc).astimezone(
                pytz.timezone("America/New_York")
            )
            day_num = end_time.timetuple().tm_yday % 46 + 1
            img_path = (
                app.root_path + "/../assets/images/animals/animal%d.jpg" % day_num
            )
    ...
    if g.patient:
        if record.uniq_id != g.patient.id:
            return abort(
                401, "Your device isn't authorized to view this completion record."
            )
        else:
            if record.completed:
                end_time = record.end_time.replace(tzinfo=pytz.utc).astimezone(
        ...
        return abort(
            401,
            "Your device appears to be unregistered. Only registered devices can view completion records.",
        )


def save_response(
    formdata, question_id, session_id=None, current_user=None, survey_response_id=None
):
...
def add_question():
    ...
    if _id == None:
        return abort(
            404,
            "Could not find survey matching that ID when adding question. Perhaps it was deleted?",
        )
    survey = Survey.query.get_or_404(_id)
    survey_id = survey.id
    ...
    if request.method == "POST" and formobj.validate():
        ...
        dbobj.user_id = current_user.id
        dbobj.choices = json.dumps(
            {str(ix): a for ix, a in enumerate(request.form.getlist("choices"))}
        )
    ...
    return render_template(
        "form.html", action="Add", data_type="a question", form=formobj
    )
...
def edit_question(_id):
    ...
    return render_template(
        "form.html", action="Edit", data_type="question " + curpos, form=formout
    )
...
def patient_signup(survey_id):
    ...
    if formobj.validate_on_submit():
        ...
        db_session.commit()
        flash(
            """You have registered this device and browser to take screenings.\n
		      ...
		      Please keep this ID for your records: %s"""
            % fmt_id(g.patient_ident)
        )
        return redirect(url_for("start_survey", survey_id=survey_id))
    if new_patient:
        return render_template(
            "form_signup_register.html",
            action="Register",
            data_type="device",
            form=formobj,
        )
...
def edit_patient(id=None):
    ...
    return render_template(
        "form.html", action="Edit", data_type="student", form=formobj
    )
...
def edit_patient_self():
    ...
    abort(
        403,
        "Editing your own information is temporarily disabled, please check back later.",
    )
    return redirect(url_for("view_patient_self"))
...
def view_patients():
    ...
    for p in patients:
        ...
        taken = (
            p.surveys.filter(
                SurveyResponse.end_time.isnot(None), SurveyResponse.start_time > today
            )
...
def patient_survey_selector(id):
    ...
    for s in surveys:
        try:
            ...
            s.description_html = "<p>" + s.description + "</p>"
    return render_template("surveys_selector.html", surveys=surveys, patient=patient)
...
def add_trigger():
    ...
    return render_template(
        "form_trigger.html", action="Add", data_type="a trigger", form=formobj
    )
...
def edit_trigger(_id):
    ...
    if request.method == "POST" and formobj.validate():
        ...
        formobj.dest_yes.data = (
            Question.query.get(trigger.dest_yes) if trigger.dest_yes else None
        )
        formobj.dest_no.data = (
            Question.query.get(trigger.dest_no) if trigger.dest_no else None
        )
    ...
    # formobj.questions.choices = [(q.id, "(ID: %s) "% str(q.id) + q.body) for q in Question.query]
    return render_template(
        "form_trigger.html", action="Edit", data_type="trigger", form=formobj
    )
...
def view_patient(id):
    ...
    if taken != None:
        ...
        if taken.exited:
            ptstat = -1
    return render_template(
        "view_patient.html", patients_feed=patients_feed, patient=p, status=ptstat
    )
...
def view_patient_self():
    if g.patient == None:
        return abort(
            401, "Please register your device first, then come back to this page."
        )
    p = g.patient
    patients_feed = []
    ...
    return render_template(
        "view_patient_self.html", patients_feed=patients_feed, patient=p, status=status
    )
...
def survey_response_dashboard(survey_id):
    ...
    dash_figs = []
    question_figs = []
    sr = survey.responses.filter(
        models.SurveyResponse.start_time > (datetime.datetime.now()).date()
    ).all()
    responses = []
    for sre in sr:
        responses.extend([r.to_dict() for r in sre.responses])
    sr = survey.responses.filter(
    ...
    responses_last7 = []
    for sre in sr:
        responses_last7.extend([r.to_dict() for r in sre.responses])
    ...
    if len(sres) > 0:
        ...
        sres.start_time = sres.start_time.dt.tz_localize("UTC").dt.tz_convert(
            "US/Eastern"
        )
        ...
        devs.creation_time = devs.creation_time.dt.tz_localize("UTC").dt.tz_convert(
            "US/Eastern"
        )
        devs_per_day = pd.DataFrame(
            devs.groupby([devs.creation_time.dt.floor("d")])["creation_time"].count()
        )
        devs_per_day.columns = ["daily_new_devices"]
        pts = model_to_pd(models.Patient)
        pts.creation_time = pts.creation_time.dt.tz_localize("UTC").dt.tz_convert(
            "US/Eastern"
        )
        pts_per_day = pd.DataFrame(
            pts.groupby([pts.creation_time.dt.floor("d")])["creation_time"].count()
        )
        pts_per_day.columns = ["daily_registered_students"]
        res_per_day = pd.DataFrame(
            sres.groupby([sres.end_time.dt.floor("d")])["end_time"].count()
        )
        res_per_day.columns = ["daily_total_surveys"]
        comp_per_day = pd.DataFrame(
            sres[sres.completed == True]
            .groupby([sres.end_time.dt.floor("d")])["end_time"]
            .count()
        )
        comp_per_day.columns = ["daily_completed_surveys"]
        exit_per_day = pd.DataFrame(
            sres[sres.exited == True]
            .groupby([sres.end_time.dt.floor("d")])["end_time"]
            .count()
        )
        exit_per_day.columns = ["daily_exited_surveys"]
        df = pd.merge(
            pts_per_day, res_per_day, left_index=True, right_index=True, how="outer"
        )
        ...
        df = (
            df.tz_localize(None)
            .reindex(pd.date_range(begin_time, datetime.datetime.now().date()))
            .fillna(0)
            .astype(int)
        )
        ...
        df["daily_uncompleted_surveys"] = (
            df["total_registered_students"] - df["daily_total_surveys"]
        )
        df["daily_pct"] = (
            df["daily_total_surveys"] / df["total_registered_students"] * 100
        )
        ...
        outdf = []
        for i in pd.date_range(begin_time, datetime.datetime.now().date()).tz_localize(
            "US/Eastern"
        ):
            for y in years:
                pts_yr = pts_df[pts_df.year == y]
                hits = pts_yr[
                    (pts_yr.index < i + datetime.timedelta(days=1)) & (pts_yr.index > i)
                ]
                response_hits = set(
                    sres[
                        (sres.end_time < (i + datetime.timedelta(days=1)))
                        & (sres.end_time > i)
                    ].uniq_id
                )
                complete_hits = set(
        ...
        outdf = outdf[
            [
                i in pd.date_range(begin_time, datetime.datetime.now().date())
                for i in outdf["date"]
            ]
        ]
        df = (
        ...
        todaydf = (
            outdf[outdf["date"] == datetime.datetime.now().date()]
            .loc[:, ["year", "Completed", "Exited", "Not Completed"]]
            .melt(id_vars="year")
        )
        fig2 = plotlyBarplot(
            data=todaydf,
            ...
            stacked=True,
            ylabel="# Students",
            xlabel="Expected Graduation",
            title="Compliance by Year",
        ...
        df2 = df.loc[
            ...
            [
                "index",
                "daily_uncompleted_surveys",
                "daily_completed_surveys",
                "daily_exited_surveys",
            ],
        ]
        ...
        reg_per_year = plotlyBarplot(
            data=pd.DataFrame(
                pts.groupby(["year", "program"]).count()["id"]
            ).reset_index(),
            y="id",
            x="year",
            hue="program",
        ...
        reg_per_program = plotlyBarplot(
            data=pd.DataFrame(
                pts.groupby(["program", "year"]).count()["id"]
            ).reset_index(),
            y="id",
            x="program",
            hue="year",
            width=None,
            height=500,
            title="Registered by Program",
            show_legend=True,
            stacked=True,
            xlabel="Program",
            ylabel="# Students",
        )
        reg_per_location = plotlyBarplot(
            data=pd.DataFrame(pts.groupby(["location"]).count()["id"]).reset_index(),
            y="id",
            x="location",
            stacked=True,
            width=None,
            height=500,
            title="Registered by Location",
            show_legend=False,
            xlabel="Location",
            ylabel="# Students",
        )
        ...
        week_pct = (
            sum(list(df["daily_total_surveys"])[-7:])
            / sum(list(df["total_registered_students"])[-7:])
            * 100
        )
    ...
    for ix, fig in enumerate(dash_figs):
        dash_figs[ix] = offline.plot(
            fig, show_link=False, output_type="div", include_plotlyjs=False
        )
    for ix, fig in enumerate(question_figs):
        question_figs[ix] = offline.plot(
            fig, show_link=False, output_type="div", include_plotlyjs=False
        )
    for ix, fig in enumerate(last7_figs):
        last7_figs[ix] = offline.plot(
            fig, show_link=False, output_type="div", include_plotlyjs=False
        )
    return render_template(
        "dashboard.html",
        dash_figs=dash_figs,
        question_figs=question_figs,
        last7_figs=last7_figs,
...
def plotlyBarplot(
    ...
    height=400,
    colors=[
        "rgba" + str(tuple(i[0:3]))
        for i in cm.get_cmap("Set1").colors + cm.get_cmap("Set2").colors
    ],
    stacked=False,
    percent=False,
    ordered=False,
    ...
    yaxis = go.layout.YAxis(title=ylabel, automargin=True, titlefont=dict(size=12),)

    xaxis = go.layout.XAxis(
        title=xlabel, automargin=True, titlefont=dict(size=12), type=xtype
    )
    if type(margins) == dict:
        ...
        layout = go.Layout(
            autosize=True, width=width, height=height, yaxis=yaxis, xaxis=xaxis
        )
    titledict = {
        "text": title,
        "y": 0.9,
